...
<div style="background:var(--surface); min-height:calc(100vh - 56px);">
    ...
    <div
        ...
        <div>
            <h1
                ...
                <span class="material-icons-round" style="color:#00897B;">assessment</span> Relatórios — Rastreadores
            </h1>
            <p style="color:#757575; font-size:.88rem; margin:0;">Acompanhe a distribuição e movimentação dos
                rastreadores</p>
        </div>
    ...
    <div class="rep-table-wrap">
        <div
            ...
            <span class="material-icons-round" style="color:#00897B; font-size:20px;">location_on</span>
            <strong style="font-size:.95rem; color:#212121;">Rastreadores Cadastrados</strong>
        </div>
        <table>
            <thead>
                <tr>
...
<script>
    let chartInst = null;
    const badges = { 'Estoque': 'estoque', 'Enviado': 'enviado', 'Recebido': 'recebido', 'Defeito': 'defeito', 'Em Manutenção': 'manutencao' };
    ...
    async function loadReport() {
        ...
        const res = await fetch('api/trackers.php?action=reports&status=' + encodeURIComponent(status));
        ...
        // Gráfico — somente counts reais de equipamentos
        const ctx = document.getElementById('trackerChart').getContext('2d');
        if (chartInst) chartInst.destroy();
        chartInst = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: ['Em Estoque', 'Enviado', 'Recebido', 'Defeito', 'Manutenção'],
                datasets: [{
                    data: [s['Estoque'] || 0, s['Enviado'] || 0, s['Recebido'] || 0, s['Defeito'] || 0, s['Em Manutenção'] || 0],
        ...
        if (evs.length) {
            ...
            <tr>
                ...
                <td><strong>${esc(ev.model || 'Rastreador')}</strong></td>
    ...
    function exportReport() {
        const status = document.getElementById('filterStatus').value;
        window.location.href = 'api/trackers.php?action=export&status=' + encodeURIComponent(status);
    }
